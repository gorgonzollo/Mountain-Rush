...
{
    [Header("Ввод — мышь")]
    public float rotationSmoothness = 5f; // Переименовал из touchSmoothness
    public float mouseSensitivity = 0.5f;
    public int mouseButton = 0; // 0 - ЛКМ, 1 - ПКМ, 2 - СКМ
    ...
    private void OnEnable()
    {
        if (carRigidbody != null)
        {
            // linearVelocity (Unity 6) или velocity (Unity 2022 и старее)
            // Использую velocity для совместимости, если у вас Unity 6 - поменяйте на linearVelocity
#if UNITY_6000_0_OR_NEWER
            var v = carRigidbody.linearVelocity;
#else
            var v = carRigidbody.velocity;
#endif
    ...
    void Update()
    {
        ...
        currentX = Mathf.Lerp(currentX, targetX, Time.deltaTime * rotationSmoothness);
        currentY = Mathf.Lerp(currentY, targetY, Time.deltaTime * rotationSmoothness);
...
#else
        Vector3 worldVel = carRigidbody.velocity;
#endif
        Vector3 localVelocity = velocityReference.InverseTransformDirection(worldVel);
    ...
    void HandleMouseInput()
    {
        ...
        // Начало перетаскивания (без проверки UI)
