...
public class CarCameraController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (EventSystem.current == null)
            Debug.LogWarning("нет EventSystem");
    }
    ...
    void HandleMouseInput()
    {
        ...
        if (isMouseDragging && Input.GetMouseButton(mouseButton))
        {
            Vector2 pos = Input.mousePosition;

            if (blockWhenOverUI && IsMouseOverOrNearUI(pos))
            {
                lastMousePos = pos;
                return;
            }
